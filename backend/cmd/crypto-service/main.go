// Package main is the entry point for the TeslaPay Crypto Service.
// Handles crypto wallet management, token balances, buy/sell/send operations
// on the Fuse Network.
package main
...
func main() {
	cfg, err := common.LoadBaseConfig("crypto-service")
	if err != nil {
	...
	// Wire up crypto service components.
	cryptoRepo := crypto.NewRepository(db, logger)
	cryptoService := crypto.NewService(cryptoRepo, producer, logger)
	...
	go func() {
		logger.Info("crypto-service listening", zap.String("addr", srv.Addr))
