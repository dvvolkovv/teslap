// Package main is the entry point for the TeslaPay Account Service.
package main
...
func main() {
	cfg, err := common.LoadBaseConfig("account-service")
	if err != nil {
	...
	repo := account.NewRepository(db, logger)
	...
	go func() {
		logger.Info("account-service listening", zap.String("addr", srv.Addr))
