// Package main is the entry point for the TeslaPay Payment Service.
// Handles SEPA SCT, SEPA Instant, internal transfers, FX exchange,
...
func main() {
	cfg, err := common.LoadBaseConfig("payment-service")
	if err != nil {
	...
	// Wire up payment service components.
	paymentRepo := payment.NewRepository(db, logger)
	paymentService := payment.NewService(paymentRepo, producer, logger)
