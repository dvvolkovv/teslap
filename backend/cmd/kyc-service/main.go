// Package main is the entry point for the TeslaPay KYC Service.
// Handles KYC verification initiation, status checks, and Sumsub webhook processing.
// Default port: 8086 (set via PORT environment variable).
package main
...
func main() {
	cfg, err := common.LoadBaseConfig("kyc-service")
	if err != nil {
	...
	// Wire up KYC service components.
	kycRepo := kyc.NewRepository(db, logger)
	kycService := kyc.NewService(kycRepo, producer, logger)
	...
	// Public route — webhook does not require authentication.
	public := router.Group("/api/v1")
	kycHandler.RegisterWebhookRoute(public)

	// Authenticated routes.
	v1 := router.Group("/api/v1")
	kycHandler.RegisterRoutes(v1)
